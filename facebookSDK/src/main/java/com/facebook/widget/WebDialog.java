...
public class WebDialog extends Dialog {
	 ...
	 * interaction with the dialog completes, whether because the dialog finished
	 * successfully, or it was cancelled, or an error was encountered.
	 */
	 ...
	 *          this URL, but it should be a valid URL pointing to a Facebook Web
	 *          Dialog
	 */
	...
	public WebDialog(Context context, String action, Bundle parameters, int theme,
		...
		// our webview client only handles the redirect uri we specify, so just hard
		// code it here
		parameters.putString(ServerProtocol.DIALOG_PARAM_REDIRECT_URI, REDIRECT_URI);

		parameters.putString(ServerProtocol.DIALOG_PARAM_DISPLAY, DISPLAY_TOUCH);

		Uri uri = Utility.buildUri(ServerProtocol.getDialogAuthority(), ServerProtocol.getAPIVersion()
	...
	protected void onCreate(Bundle savedInstanceState) {
		 ...
		 * at this point, we only need to know its drawable width and height to
		 * place the webview
		 */
		createCrossImage();

		/*
		 * Now we know 'x' drawable width and height, layout the webview and add it
		 * the contentFrameLayout layout
		 */
	...
	private void calculateSize() {
		...
		// always use the portrait dimensions to do the scaling calculations so we
		// always get a portrait shaped
		// web dialog
		...
		int dialogWidth = Math.min(
				...
				metrics.widthPixels);
		int dialogHeight = Math
				.min(
						getScaledSize(height, metrics.density, NO_PADDING_SCREEN_HEIGHT,
								MAX_PADDING_SCREEN_HEIGHT), metrics.heightPixels);
	...
	private int getScaledSize(int screenSize, float density, int noPaddingSize, int maxPaddingSize) {
		...
		if (scaledSize <= noPaddingSize) {
			...
			// between the noPadding and maxPadding widths, we take a linear reduction
			// to go from 100%
	...
	private class DialogWebViewClient extends WebViewClient {
		...
		public void onPageFinished(WebView view, String url) {
			...
			/*
			 * Once web view is fully loaded, set the contentFrameLayout background to
	...
	private static class BuilderBase<CONCRETE extends BuilderBase<?>> {
		...
		/**
		 * Constructs a WebDialog using the parameters provided. The dialog is not
		 * shown, but is ready to be shown by calling Dialog.show().
		 * 
	...
	public static class Builder extends BuilderBase<Builder> {
		/**
		 * Constructor that builds a dialog using either the active session, or the
		 ...
		 *          https://developers.facebook.com/docs/reference/dialogs/ for
		 *          details.
		 */
	...
	public static class FeedDialogBuilder extends BuilderBase<FeedDialogBuilder> {
		...
		/**
		 * Constructor that builds a Feed Dialog using either the active session, or
		 ...
		 *          showing the dialog; must not be null, and must be opened.
		 * @param parameters
		 *          a Bundle containing parameters to pass as part of the dialog
		 *          URL. No validation is done on these parameters; it is the
		...
		/**
		 * Constructor that builds a Feed Dialog using the provided application ID
		 * and parameters.
		 * 
		 * @param context
		 *          the Context within which the dialog will be shown.
		 * @param applicationId
		 *          the application ID to use. If null, the application ID specified
		 *          in the application/meta-data will be used instead.
		 * @param parameters
		 *          a Bundle containing parameters to pass as part of the dialog
		 *          URL. No validation is done on these parameters; it is the
		 *          caller's responsibility to ensure they are valid. For more
		...
		/**
		 * Sets the ID of the profile that the story will be published to. If not
		 * specified, it will default to the same profile that the story is being
		 * published from.
		 * 
		 * @param id
		...
		/**
		 * Sets the URL of a media file attached to this post. If this is set, any
		 * picture set via setPicture will be ignored.
		 * 
		 * @param source
	...
	public static class RequestsDialogBuilder extends BuilderBase<RequestsDialogBuilder> {
		 ...
		 * session, or the application ID specified in the application/meta-data.
		 * 
		 * @param context
		 *          the Context within which the dialog will be shown.
		 */
		...
		/**
		 * Constructor that builds a Requests Dialog using the provided session and
		 * parameters.
		 * 
		 * @param context
		 *          the Context within which the dialog will be shown.
		...
		/**
		 * Constructor that builds a Requests Dialog using the provided application
		 * ID and parameters.
		 * 
		 * @param context
		 *          the Context within which the dialog will be shown.
