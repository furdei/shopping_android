...
public class GoodsStatisticsContentProvider extends
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
		...
		// Set the table
		queryBuilder.setTables(GOODS_STATISTICS_PATH);
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		long id = db.insert(GOODS_STATISTICS_PATH, null, values);

		if (id < 0)
			throw new IllegalStateException(String.format(ERROR_FAILED_TO_ADD_A_ROW,
					GOODS_STATISTICS_PATH, id));

		Uri inserted = ContentUris.withAppendedId(GOODS_STATISTICS_URI, id);
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		SQLiteDatabase db = getDbHelper().getDb();
		int rowsAffected = db.update(GOODS_STATISTICS_PATH, values, Columns._id.getDbName() + "=?",
